...
export function UserProfilePanel({ compact = false }: UserProfilePanelProps) {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      setAgeInput(json.profile.career?.age ? String(json.profile.career.age) : "");
      setEducationInput(json.profile.career?.education ?? "");
      setCollegeInput(json.profile.career?.college ?? "");
      setDegreeInput(json.profile.career?.degree ?? "");
      setSkillsInput(json.profile.career?.skills?.join(", ") ?? "");
      setInterestsInput(json.profile.career?.interests?.join(", ") ?? "");
      setCareerGoalsInput(json.profile.career?.careerGoals?.join(", ") ?? "");
      setLinkedinInput(json.profile.career?.linkedinUrl ?? "");
      setGithubInput(json.profile.career?.githubUrl ?? "");
      setMessage("");
    } finally {
      setLoading(false);
    }
  ...
  async function saveProfile() {
    ...
    try {
      ...
      body.age = ageInput.trim() ? Number(ageInput) : null;
      body.education = educationInput.trim() || null;
      body.college = collegeInput.trim() || null;
      body.degree = degreeInput.trim() || null;
      body.skills = skillsInput.split(",").map((item) => item.trim()).filter(Boolean);
      body.interests = interestsInput.split(",").map((item) => item.trim()).filter(Boolean);
      body.careerGoals = careerGoalsInput.split(",").map((item) => item.trim()).filter(Boolean);
      body.linkedinUrl = linkedinInput.trim() || null;
      body.githubUrl = githubInput.trim() || null;
        ...
        current
          ? {
              ...
              career: {
                ...
                resumeFileId: json.profile.resumeFileId,
                linkedinUrl: json.profile.linkedinUrl,
                githubUrl: json.profile.githubUrl,
                profileCompletion: json.profile.profileCompletion,
                careerReadiness: json.profile.careerReadiness,
              },
            }
          : current,
      );
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
          <div className="soft-card p-5">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
              {[
                { label: "Profile completion", value: profile.career?.profileCompletion ?? 0 },
                { label: "Career readiness", value: profile.career?.careerReadiness ?? 0 },
              ].map((item) => (
                <div key={item.label}>
                  ...
                  <div className="h-2.5 rounded-full bg-white/70">
                    <div
                      className="h-full rounded-full bg-gradient-to-r from-emerald-300 via-sky-300 to-indigo-300 transition-[width] duration-700"
            ...
            <p className="mt-4 text-sm text-slate-500">
              Complete education, skills, goals, resume, LinkedIn, and GitHub to improve recommendations.
            </p>
          </div>

          <div className="soft-card p-5">
            <p className="sub-title">Career profile</p>
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <input className="input" placeholder="Age" value={ageInput} onChange={(e) => setAgeInput(e.target.value)} />
              <input className="input" placeholder="Education level" value={educationInput} onChange={(e) => setEducationInput(e.target.value)} />
              <input className="input" placeholder="College" value={collegeInput} onChange={(e) => setCollegeInput(e.target.value)} />
              <input className="input" placeholder="Degree" value={degreeInput} onChange={(e) => setDegreeInput(e.target.value)} />
              <input className="input md:col-span-2" placeholder="Skills, comma separated" value={skillsInput} onChange={(e) => setSkillsInput(e.target.value)} />
              <input className="input md:col-span-2" placeholder="Interests, comma separated" value={interestsInput} onChange={(e) => setInterestsInput(e.target.value)} />
              <input className="input md:col-span-2" placeholder="Career goals, comma separated" value={careerGoalsInput} onChange={(e) => setCareerGoalsInput(e.target.value)} />
              <input className="input" placeholder="LinkedIn URL" value={linkedinInput} onChange={(e) => setLinkedinInput(e.target.value)} />
              <input className="input" placeholder="GitHub URL" value={githubInput} onChange={(e) => setGithubInput(e.target.value)} />
            </div>
